<template>
  <div class="min-h-screen bg-yellow-50 flex items-center justify-center p-4">
    <div v-if="isTelegramLoading" class="text-center">
       <div class="animate-spin rounded-full h-12 w-12 border-b-4 border-black mx-auto mb-4"></div>
       <p class="font-black uppercase tracking-widest">Авторизація через Telegram...</p>
    </div>

    <div v-else class="max-w-md w-full bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-8">
...
onMounted(async () => {
  ...
  if (tg && tg.initData && tg.initData.length > 0) {
    ...
    const success = await auth.loginViaTelegram(tg.initData);

    if (success) {
      router.push('/');
    } else {
      isTelegramLoading.value = false;
      error.value = "Не вдалося увійти через Telegram. Увійдіть за допомогою пароля.";
    }
  }
});
